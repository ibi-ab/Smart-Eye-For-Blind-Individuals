...
def prompt_command():
    print("If you want to detect objects, say 'Object Detection'. "
                   "If you want to know about the pose of the person, say 'Pose Estimation'. "
                   "For reading, say 'OCR'.")
    tts_engine.say("If you want to detect objects, say 'Object Detection'. "
                   "If you want to know about the pose of the person, say 'Pose Estimation'. "
                   "For reading, say 'OCR'.")
    tts_engine.runAndWait()
    ...
    while True:
        ...
        if 'object detection' in command:
            return 'A'
        elif 'pose estimation' in command:
            return 'B'
        elif 'ocr' in command:
            return 'C'
        else:
            print("Command not recognized. Please say 'Object Detection', 'Pose Estimation', or 'OCR'.")
            tts_engine.say("Command not recognized. Please say 'Object Detection', 'Pose Estimation', or 'OCR'.")
            tts_engine.runAndWait()
...
def main():
    ...
    while True:
        if current_choice is None:
            choice = prompt_command()
        else:
            choice = current_choice

        if choice == 'A':  # Object Detection
            while True:
                ret, frame = cap.read()
                if not ret:
                    break
                ...
                if 'quit object detection' in voice_command:
            ...
            while True:
                ...
                voice_command = recognize_speech()
                if 'quit pose estimation' in voice_command:
                    cv2.destroyAllWindows()
                    current_choice = None
                    break
